...
void FUNC(crum_analyze)(VAR *array, VAR *swap, size_t swap_size, size_t nmemb, CMPFUNC *cmp)
{
	...
	O("crum_analyze: ss=%zu, n=%zu, half1=%zu, quad1=%zu, quad2=%zu, half2=%zu, quad3=%zu, quad4=%zu\n",
		swap_size, nmemb,
		half1, quad1, quad2, half2, quad3, quad4);
	...
	O("crum_analyze: abal=%zu, bbal=%zu, cbal=%zu, dbal=%zu\n", abalance, bbalance, cbalance, dbalance);
	O("crum_analyze: quad1=%zu, quad2=%zu, quad3=%zu, quad4=%zu\n", quad1, quad2, quad3, quad4);
	if (cnt == 0)
	{
    ...
    O("crum_analyze: init sum: asum=%zu, bsum=%zu, csum=%zu, dsum=%zu\n", asum, bsum, csum, dsum);
	if (asum | bsum | csum | dsum)
	{
        ...
        O("crum_analyze: branch2: span1=%zu, span2=%zu, span3=%zu\n", span1, span2, span3);
...
//		asum = bsum = csum = dsum = 1;
	...
	int sum = asum + bsum * 2 + csum * 4 + dsum * 8;
    O("crum_analyze: branch3: asum=%zu, bsum=%zu, csum=%zu, dsum=%zu\n", asum, bsum, csum, dsum);
	switch (sum)
	{
		case 0:
	...
	O("crum_analyze: final; pa=%zu, pb=%zu, pc=%zu, pd=%zu\n", pta - array, ptb - array, ptc - array, ptd - array);
...
{
    O("crum_median_of_cbrt: ss=%zu, n=%zu\n", swap_size, nmemb);
	VAR *pta, *piv;
	size_t cnt, cbrt, div;
	...
	*generic = (cmp(piv + cbrt * 2 - 1, piv) <= 0) & (cmp(piv + cbrt - 1, piv) <= 0);
	...
	VAR *r = FUNC(crum_binary_median)(piv, piv + cbrt, cbrt, cmp);
	O("crum_median_of_cbrt: result=%zu\n", *r);
	return r;
}

size_t FUNC(crum_median_of_three)(VAR *array, size_t v0, size_t v1, size_t v2, CMPFUNC *cmp)
{
    O("crum_median_of_three: v0=%zu, v1=%zu, v2=%zu\n", v0, v1, v2);
	size_t v[3] = {v0, v1, v2};
	char x, y, z;
...
VAR *FUNC(crum_median_of_nine)(VAR *array, size_t nmemb, CMPFUNC *cmp)
{
	...
	VAR *r = array + FUNC(crum_median_of_three)(array, x, y, z, cmp);
	O("crum_median_of_nine: result=%zu\n", *r);
	return r;
}

size_t FUNC(fulcrum_default_partition)(VAR *array, VAR *swap, VAR *ptx, VAR *piv, size_t swap_size, size_t nmemb, CMPFUNC *cmp)
{
    O("fulcrum_default_partition: ss=%zu, n=%zu, pivot=%zu\n", swap_size, nmemb, *piv);
	...
	O("fulcrum_default_partition: m=%zu\n", m);
	return m;
}
...
{
    O("fulcrum_reverse_partition: ss=%zu, n=%zu, pivot=%zu\n", swap_size, nmemb, *piv);
	size_t i, cnt, val, m = 0;
	VAR *ptl, *ptr, *pta, *tpa;
	...
	O("fulcrum_reverse_partition: m=%zu\n", m);
	return m;
}

void FUNC(fulcrum_partition)(VAR *array, VAR *swap, VAR *max, size_t swap_size, size_t nmemb, CMPFUNC *cmp)
{
    O("fulcrum_partition: ss=%zu, n=%zu\n", swap_size, nmemb);
