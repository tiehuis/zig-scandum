...
void FUNC(blit_analyze)(VAR *array, VAR *swap, size_t swap_size, size_t nmemb, CMPFUNC *cmp)
{
	...
	O("blit_analyze: ss=%zu, n=%zu, half1=%zu, quad1=%zu, quad2=%zu, half2=%zu, quad3=%zu, quad4=%zu\n",
		swap_size, nmemb,
	...
	O("blit_analyze: abal=%zu, bbal=%zu, cbal=%zu, dbal=%zu\n", abalance, bbalance, cbalance, dbalance);
	O("blit_analyze: quad1=%zu, quad2=%zu, quad3=%zu, quad4=%zu\n", quad1, quad2, quad3, quad4);
	if (cnt == 0)
	{
    ...
    O("blit_analyze: init sum: asum=%zu, bsum=%zu, csum=%zu, dsum=%zu\n", asum, bsum, csum, dsum);
	if (asum | bsum | csum | dsum)
	{
        ...
        O("blit_analyze: branch2: span1=%zu, span2=%zu, span3=%zu\n", span1, span2, span3);
...
#endif
    O("blit_analyze: branch3: asum=%zu, bsum=%zu, csum=%zu, dsum=%zu\n", asum, bsum, csum, dsum);
	...
	O("blit_analyze: final; pa=%zu, pb=%zu, pc=%zu, pd=%zu\n", pta - array, ptb - array, ptc - array, ptd - array);
...
{
    O("blit_binary_median: l=%zu\n", len);
	while (len /= 2)
	{
		if (cmp(pta + len, ptb + len) <= 0) pta += len; else ptb += len;
	}
	VAR r = cmp(pta, ptb) > 0 ? *pta : *ptb;
	O("blit_binary_median: result=%zu\n", r);
	return r;
}
...
VAR FUNC(blit_median_of_cbrt)(VAR *array, VAR *swap, size_t swap_size, size_t nmemb, int *generic, CMPFUNC *cmp)
{
	...
	*generic = cmp(pts + cbrt * 2 - 1, pts) <= 0;
	...
	VAR r = FUNC(blit_binary_median)(pts, pts + cbrt, cbrt, cmp);
	O("blit_median_of_cbrt: result=%zu\n", r);
	return r;
}
...
{
    O("blit_reverse_partition: ss=%zu, n=%zu, pivot=%zu\n", swap_size, nmemb, *piv);
	if (nmemb > swap_size)
	{
...
{
    O("blit_default_partition: ss=%zu, n=%zu, pivot=%zu\n", swap_size, nmemb, *piv);
	if (nmemb > swap_size)
	{
		size_t l, r, h = nmemb / 2;
