...
void FUNC(flux_analyze)(VAR *array, VAR *swap, size_t swap_size, size_t nmemb, CMPFUNC *cmp)
{
	...
	O("flux_analyze: ss=%zu, n=%zu, half1=%zu, quad1=%zu, quad2=%zu, half2=%zu, quad3=%zu, quad4=%zu\n",
		swap_size, nmemb,
	...
	O("flux_analyze: abal=%zu, bbal=%zu, cbal=%zu, dbal=%zu\n", abalance, bbalance, cbalance, dbalance);
	O("flux_analyze: quad1=%zu, quad2=%zu, quad3=%zu, quad4=%zu\n", quad1, quad2, quad3, quad4);
	if (cnt == 0)
	{
    ...
    O("flux_analyze: init sum: asum=%zu, bsum=%zu, csum=%zu, dsum=%zu\n", asum, bsum, csum, dsum);
	if (asum | bsum | csum | dsum)
	{
        ...
        O("flux_analyze: branch2: span1=%zu, span2=%zu, span3=%zu\n", span1, span2, span3);
    ...
    O("flux_analyze: branch3: asum=%zu, bsum=%zu, csum=%zu, dsum=%zu: sum=%zu\n", asum, bsum, csum, dsum, sum);
	O("flux_analyze: branch3: abal=%zu, bbal=%zu, cbal=%zu, dbal=%zu\n", abalance, bbalance, cbalance, dbalance);
	switch (sum)
	{
		case 0:
	...
	O("flux_analyze: final; pa=%zu, pb=%zu, pc=%zu, pd=%zu\n", pta - array, ptb - array, ptc - array, ptd - array);
...
{
    O_RAW("|blit| blit_binary_median: l=%zu\n", len);
	while (len /= 2)
	{
...
{
    O_RAW("|blit| blit_median_of_nine: n=%zu\n", nmemb);
	VAR *pta, swap[9];
	size_t x, y, z;
...
{
    O("flux_reverse_partition: n=%zu, pivot=%zu\n", nmemb, *piv);
...
{
    O("flux_default_partition: n=%zu, pivot=%zu\n", nmemb, *piv);
	...
	O("flux_default_partition: run=%zu, m=%zu\n", run, m);
...
void FUNC(flux_partition)(VAR *array, VAR *swap, VAR *ptx, VAR *piv, size_t nmemb, CMPFUNC *cmp)
{
	...
	while (1)
	{
		...
		s_size = nmemb - a_size;
		O("flux_partition: a_size=%zu, s_size=%zu\n", a_size, s_size);
